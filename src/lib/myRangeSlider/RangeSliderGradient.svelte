		...
		>
			...
			{#if float}
				...
				{#if activeHandle === index && focus && !colorPickerOpen}
					<button class="pickerButton h-6 w-6" on:click={toggleColorPicker}>
						<img src="src/lib/icons/color.svg" class="h-6 w-6" alt="Open color picker" />
					</button>
				{/if}
			{/if}
		</span>
...
{#each colors as colorTuple, index}
	<div class={index === activeHandle && colorPickerOpen ? '' : 'hidden'}>
		<button
			on:click={() => {
				console.log(colors);
